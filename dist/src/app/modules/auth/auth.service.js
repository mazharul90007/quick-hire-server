...
const createApplicant = async (payload) => {
    ...
    try {
        ...
        throw new ApiError(status.INTERNAL_SERVER_ERROR, "Failed to complete applicant registration. Please try again.");
    }
};
//====================Create Admin====================
...
const forgetPassword = async (payload) => {
    ...
    if (user && !user.emailVerified) {
        throw new ApiError(status.FORBIDDEN, "Please verify your email address first!");
    }
...
const createRecruiter = async (payload) => {
    ...
    try {
        ...
        await prisma.user.delete({ where: { id: userId } });
        throw new ApiError(status.INTERNAL_SERVER_ERROR, "Recruiter registration failed!");
    }
};
