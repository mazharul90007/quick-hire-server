...
async function ensureSubIndustryMatchesIndustry(industryId, subIndustryId) {
    ...
    if (sub.industryId !== industryId) {
        throw new ApiError(status.BAD_REQUEST, "Sub-industry does not belong to the given industry");
    }
}
...
export async function updateMyProfile(userId, data, files) {
    ...
    await ensureSubIndustryMatchesIndustry(data.industryId ?? undefined, data.subIndustryId ?? undefined);
    const userData = {};
    if (data.recruiterName !== undefined)
        userData.name = data.recruiterName;
    if (files.image)
        userData.image = files.image;
    const recData = {};
    if (data.recruiterName !== undefined)
        recData.recruiterName = data.recruiterName;
    if (data.recruiterPhone !== undefined)
        recData.recruiterPhone = data.recruiterPhone;
    if (data.recruiterWorkEmail !== undefined) {
        recData.recruiterWorkEmail = data.recruiterWorkEmail;
    }
    if (data.companyName !== undefined)
        recData.companyName = data.companyName;
    if (data.companyWebsite !== undefined)
        recData.companyWebsite = data.companyWebsite;
