...
const createIndustry = catchAsync(async (req, res) => {
    ...
    if (!file?.buffer?.length) {
        throw new ApiError(status.BAD_REQUEST, "Industry logo image is required (multipart field: logo)");
    }
...
const updateIndustry = catchAsync(async (req, res) => {
    ...
    if (body.name === undefined && !file?.buffer?.length) {
        throw new ApiError(status.BAD_REQUEST, "Provide a new name and/or a logo image to update");
    }
    let logoUrl;
    if (file?.buffer?.length) {
        logoUrl = await uploadProfileImage(file.buffer, INDUSTRY_LOGO_FOLDER);
    }
    const result = await IndustryService.updateIndustry(id, {
