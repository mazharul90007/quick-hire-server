...
const createApplication = async (userId, payload) => {
    ...
    if (job.status !== "ACTIVE") {
        throw new ApiError(status.BAD_REQUEST, "This job is not accepting applications");
    }
    ...
    if (existing) {
        throw new ApiError(status.CONFLICT, "You have already applied for this job");
    }
...
const getAllApplications = async (filters, options, viewer) => {
    ...
    const whereConditions = parts.length > 0 ? { AND: parts } : {};
...
const getSingleApplication = async (id, viewer) => {
    ...
    if (viewer.role === UserRole.RECRUITER) {
        ...
        if (result.job.recruiterId !== recruiter.id) {
            throw new ApiError(status.FORBIDDEN, "You can only view applications for jobs you have posted");
        }
        return result;
    }
    ...
    if (viewer.role === UserRole.APPLICANT) {
        ...
        if (result.applicantId !== applicant.id) {
            throw new ApiError(status.FORBIDDEN, "You can only view applications you created");
        }
        return result;
    }
