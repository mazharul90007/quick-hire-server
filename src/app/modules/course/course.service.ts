...
function computeAccessExpiresAt(
  paidAt: Date,
  duration: CourseAccessDuration,
): Date | null {
  if (duration === CourseAccessDuration.UNLIMITED) return null;
  const d = new Date(paidAt);
  if (duration === CourseAccessDuration.MONTHS_6) d.setMonth(d.getMonth() + 6);
  if (duration === CourseAccessDuration.MONTHS_12) d.setMonth(d.getMonth() + 12);
  return d;
}
...
const getPublishedCourse = async (idOrSlug: string) => {
...
type CourseAdminRowPayload = Prisma.CourseGetPayload<{
  include: typeof courseAdminInclude;
}>;

function toCourseAdminApiRow(row: CourseAdminRowPayload) {
...
async function requireAdminIdForUser(userId: string): Promise<string> {
  const admin = await prisma.admin.findUnique({ where: { userId } });
  if (!admin) {
    throw new ApiError(
      status.FORBIDDEN,
      "An admin profile is required to create or manage courses.",
    );
  }
  return admin.id;
}
...
//======== Create course =========
const createCourse = async (userId: string, payload: CreateCourseInput) => {
  ...
  const row = await prisma.course.create({
    data: {
      ...
      accessDuration: payload.accessDuration as CourseAccessDuration,
...
const updateCourse = async (courseId: string, payload: UpdateCourseInput) => {
  ...
  if (payload.priceAmount !== undefined) data.priceAmount = payload.priceAmount;
  if (payload.currency !== undefined) data.currency = payload.currency;
  if (payload.accessDuration !== undefined) {
    data.accessDuration = payload.accessDuration as CourseAccessDuration;
  }
...
const getApplicantByUserId = async (userId: string) => {
...
const createCheckoutSession = async (userId: string, courseId: string) => {
  ...
  const stripeMinorAmount = coursePriceToStripeMinorUnits(
    course.priceAmount,
    course.currency,
  );
...
const listMyPurchases = async (userId: string) => {
  const applicant = await getApplicantByUserId(userId);
...
const getMyPaidPurchase = async (userId: string, purchaseId: string) => {
  const applicant = await getApplicantByUserId(userId);
...
//======== Build receipt PDF =========
const buildReceiptPdfBuffer = async (purchaseId: string, userId: string): Promise<Buffer> => {
...
const fulfillCheckoutSession = async (session: import("stripe").Stripe.Checkout.Session) => {
  const purchaseId = session.client_reference_id ?? session.metadata?.purchaseId;
  if (!purchaseId || !session.id) {
    console.warn(
      "[course-webhook] skip: missing purchaseId or session.id",
      JSON.stringify({
  ...
  if (session.payment_status !== "paid") {
    console.warn(
      "[course-webhook] skip: payment_status is not paid:",
  ...
  if (stripeMinor != null && stripeMinor !== expectedMinor) {
    console.error(
      "[course-webhook] skip: amount mismatch — Stripe amount_total",
      stripeMinor,
      "DB purchase.amount",
      expectedMinor,
      "purchaseId",
