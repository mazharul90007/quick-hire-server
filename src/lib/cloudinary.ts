...
function uploadRawPdf(buffer: Buffer, folder: string): Promise<string> {
  ensureConfig();
...
export function uploadApplicationCvPdf(buffer: Buffer): Promise<string> {
  return uploadRawPdf(buffer, "quickhire/application-cvs");
}

/** Applicant profile CV (PDF), separate folder from job applications. */
export function uploadApplicantCvPdf(buffer: Buffer): Promise<string> {
